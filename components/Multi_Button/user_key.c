...
static void short_pressed_cb(uint8_t pin_no, uint8_t key_action)
{
  ...
  case APP_KEY_PUSH:
    ...
    esp_timer_start_once(gs_m_key_time_params.long_press_time_handle2, s_m_key_config->long_pressed_time2);
  ...
  case APP_KEY_RELEASE:
    ...
    esp_timer_stop(gs_m_key_time_params.long_press_time_handle2);
...
static void long_press_time_out_handle2(void *arg)
{
  key_config_t *s_key_config = (key_config_t *)arg;  
  gs_m_key_param.long_press_callback2(s_key_config->key_number,&(s_key_config->short_pressed_counts)); 
}
/** 
...
int32_t user_key_init(key_config_t *key_config,
                      ...
                      user_key_function_callback_t long_pressed_cb2,
...
{  
  ...
  {    
    ...
    gpio_isr_handler_add((key_config + i)->key_number, gpio_intr_handler, (void*)(intptr_t)((key_config + i)->key_number));
  }    
  /* 依次打开对应IO的中断 */  
  for (uint8_t i = 0; i < key_counts;i++)
  {
  ...
  err_code = xTaskCreate(gpio_intr_task_thread, "gpio_intr_task_thread", 256*8, NULL, 8 , NULL);
  if(err_code != pdPASS)
  {
  ...
  esp_timer_create_args_t esp_long_press_timer_args2 = 
  {
    .callback         = long_press_time_out_handle2,
    .arg              = key_config,
    .dispatch_method  = ESP_TIMER_TASK,
    .name             = "esp_long_press_timer2",
  };
  err_code = esp_timer_create(&esp_long_press_timer_args2, &gs_m_key_time_params.long_press_time_handle2);
  if (err_code != ESP_OK)
  {
    ESP_LOGI("user_key_init", "esp_long_press_timer2 is %d\n", err_code);
    return err_code;
  }
  /* 填充按键长按所需要的时间相关参数并创建对应的计时函数 */
